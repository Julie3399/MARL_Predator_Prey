...
def save_maddpg_prey(maddpg_agent, directory):
    """
    Saves the MADDPG agent's actor and critic networks.
    """
    if not os.path.exists(directory):
    ...
    for idx, policy_net in enumerate(maddpg_agent.predator_policy_nets):
        torch.save(policy_net.state_dict(), os.path.join(directory, f'prey_policy_net_{idx}.pth'))

    torch.save(maddpg_agent.predator_q_net.state_dict(), os.path.join(directory, 'prey_q_net.pth'))

    for idx, target_policy_net in enumerate(maddpg_agent.target_predator_policy_nets):
        torch.save(target_policy_net.state_dict(), os.path.join(directory, f'target_prey_policy_net_{idx}.pth'))

    torch.save(maddpg_agent.target_predator_q_net.state_dict(), os.path.join(directory, 'target_prey_q_net.pth'))
