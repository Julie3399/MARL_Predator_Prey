...
def load_maddpg_prey(maddpg_agent, directory):
    """
    Loads the MADDPG agent's actor and critic networks from saved state dictionaries.
    """
    for idx, policy_net in enumerate(maddpg_agent.predator_policy_nets):
        policy_net.load_state_dict(torch.load(os.path.join(directory, f'prey_policy_net_{idx}.pth')))

    maddpg_agent.predator_q_net.load_state_dict(torch.load(os.path.join(directory, 'prey_q_net.pth')))

    for idx, target_policy_net in enumerate(maddpg_agent.target_predator_policy_nets):
        target_policy_net.load_state_dict(torch.load(os.path.join(directory, f'target_prey_policy_net_{idx}.pth')))

    maddpg_agent.target_predator_q_net.load_state_dict(torch.load(os.path.join(directory, 'target_prey_q_net.pth')))