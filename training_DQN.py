...
dqn_agent_predator_1 = DQNAgent(state_size=env.observation_space("predator_1").shape[0], action_size=env.action_space("predator_1").n, seed=2)
# dqn_agent_predator_2 = DQNAgent(state_size=env.observation_space("predator_2").shape[0], action_size=env.action_space("predator_2").n, seed=3)
dqn_agent_prey_0 = DQNAgent(state_size=env.observation_space("prey_0").shape[0], action_size =env.action_space("prey_0").n, seed=4)
dqn_agent_prey_1 = DQNAgent(state_size=env.observation_space("prey_1").shape[0], action_size=env.action_space("prey_1").n, seed=5)
dqn_agent_prey_2 = DQNAgent(state_size=env.observation_space("prey_2").shape[0], action_size=env.action_space("prey_2").n, seed=6)
dqn_agent_prey_3 = DQNAgent(state_size=env.observation_space("prey_3").shape[0], action_size=env.action_space("prey_3").n, seed=7)

# Initialize wandb
wandb.init(project='MAPP_version2_DQN_DQN', name='DQN_DQN_2_4')
...
save_dir = 'DQN_models_2_4'  # Make sure this directory exists or create it
...
for episode in tqdm(range(NUM_EPISODES), desc="Training Episodes"):
    ...
    while env.agents:
        ...
        for agent, obs in observations.items():
            ...
            elif "prey_2" in agent:
                actions[agent] = dqn_agent_prey_2.act(obs, eps)
            elif "prey_3" in agent:
                actions[agent] = dqn_agent_prey_3.act(obs, eps)
            # else:
            #     actions[agent] = dqn_agent_prey_0.act(obs, eps)
        ...
        for agent, obs in observations.items():
            ...
            if "predator_0" in agent:
                ...
                dqn_agent_prey_2.step(obs, action, reward, next_obs, done)
            elif "prey_3" in agent:
                dqn_agent_prey_3.step(obs, action, reward, next_obs, done)
            # else:
            #     dqn_agent_prey_0.step(obs, action, reward, next_obs, done)
    ...
    if episode == NUM_EPISODES - 1:
        ...
        save_dqn(dqn_agent_prey_1, 'dqn_agent_prey_1', save_dir)
        save_dqn(dqn_agent_prey_2, 'dqn_agent_prey_2', save_dir)
        save_dqn(dqn_agent_prey_3, 'dqn_agent_prey_3', save_dir)
