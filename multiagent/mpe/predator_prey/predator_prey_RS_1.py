...
class Scenario(BaseScenario):
    ...
    def predator_reward(self, agent, world, shape=True, closer=False):
        ...
        def collaborative_reward(agent, other_agents, collaboration_radius=1.0, bonus=0.1, penalty=-0.1, closer=False):
            ...
            if closer:
                count_nearby = sum(1 for other in other_agents if
                                   np.linalg.norm(agent.state.p_pos - other.state.p_pos) < collaboration_radius)
                return bonus * count_nearby
            else:
                # Reward for staying apart from other agents
                count_too_close = sum(1 for other in other_agents if
                                      np.linalg.norm(agent.state.p_pos - other.state.p_pos) < collaboration_radius)
                return penalty * count_too_close

        def incremental_distance_reward(agent, preys, factor=-0.1):
            # Reward predators based on increasing distance to the closest prey
            min_distance = min(np.sqrt(np.sum(np.square(agent.state.p_pos - adv.state.p_pos))) for adv in preys)
        ...
        if shape:
            ...
            rew += incremental_distance_reward(agent, agents)
            # rew += exploration_bonus(agent, world.visited_positions)
            rew += collaborative_reward(agent, predators, closer=closer)
